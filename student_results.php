...
$term = strtolower(trim($_GET['term']));  // e.g. "term 1"
...
// Fetch results for 8-4-4 subjects
$result_stmt = $conn->prepare("SELECT subject, cat_marks, exam_marks FROM results 
    WHERE admission_no = ? AND term = ? AND year = ? 
    AND subject IN (SELECT name FROM subject WHERE curriculum = '8-4-4')");
...
foreach ($subjects as $subject) {
    if (array_key_exists($subject['category'], $subjectCategories)) {
...
$groupedSubjects = [
    'science' => [], 'humanities' => [],
    'language' => [], 'technology' => [],
    'business' => [], 'personal_dev' => []
];
$mathSubject = null;

foreach ($enriched as $e) {
    if (strtolower($e['subject']) === 'mathematics') {
        $mathSubject = $e;
    } else {
        foreach ($subjectCategories as $category => $names) {
            if (in_array($e['subject'], $names)) {
                $groupedSubjects[$category][] = $e;
            }
        }
    }
}
foreach ($groupedSubjects as &$subList) {
    usort($subList, fn($a, $b) => $b['points'] <=> $a['points']);
}
...
$groupedSubjects['science'] = array_slice($groupedSubjects['science'], 2);
foreach (['English', 'Kiswahili'] as $lang) {
    foreach ($groupedSubjects['language'] as $key => $subject) {
        if ($subject['subject'] === $lang) {
            $bestSubjects[] = $subject;
            unset($groupedSubjects['language'][$key]);
        }
    }
}
if (!empty($groupedSubjects['humanities'])) {
    $bestSubjects[] = array_shift($groupedSubjects['humanities']);
}
...
<head>
    ...
    <style>
        ...
        .bottom-nav {
            ...
            background: #003366; display: flex;
            justify-content: space-around; padding: 8px 0; z-index: 999;
        }
        .bottom-nav a {
        ...
        @media print {
            .buttons, .bottom-nav, footer, .top-nav { display: none; }
        }
    </style>
</head>
<body>
...
<div class="slip-container">
    ...
    <div class="footer-sign">
        ...
        <div>Stamp<br><br>___________________</div>
    </div>
...
<div class="bottom-nav">
   <a href="select_result.php" class="card"><i class="fas fa-file-alt"></i> Results</a>
        <a href="student_media_gallery.php" class="card"><i class="fas fa-photo-video"></i> Media Gallery</a>
        <a href="student_news.php" class="card"><i class="fas fa-newspaper"></i> News & Events</a>
        <a href="student_contact.php" class="card"><i class="fas fa-phone-alt"></i> Contact School</a>
        <a href="student_profile.php" class="card"><i class="fas fa-user-cog"></i> Profile</a>
        <a href="cbc_tracker.php" class="card"><i class="fas fa-brain"></i> CBC Skills</a>
        <a href="student_fee.php" class="card"><i class="fas fa-money-check-alt"></i> Fee Portal</a>
</div>
